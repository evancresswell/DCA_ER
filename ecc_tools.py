...
# -------------------------------------------------------------------------------------------------------------------- #
 ...
 #
def npy2fa(pfam_id, npy_infile, pdb_ref_file, ipdb, preprocess=False, gap_seqs=.2, gap_cols=.2, prob_low=.004, conserved_cols=.9, letter_format=True, first_10=False):
...
def npy2fa_new(data_path, pfam_id, pdb_data_dir, index_pdb=0, n_cpu=4, create_new=True, processed_data_dir='./'): # letter_format is True
    # creates output for pydca to use
    # (output generated from .npy MSA files generated by Pfam parsing)
...
#     s = np.load('%s/msa.npy' % msa_dir).T
    ...
    = contact_map_new(pdb_id=pdb_select['PDB ID'][:4], pdb_range=[pdb_select['Subject Beg'], pdb_select['Subject End']], \
                  removed_cols=removed_cols, queried_seq=pdb_select['Subject Aligned Seq'], mismatches=pdb_select['Mismatches'], pdb_out_dir=pdb_data_dir)
    ...
    from data_processing import remove_bad_seqs
    # removes all sequences (rows) with >gap_seqs gap %
    s, tpdb = remove_bad_seqs(s, tpdb, .2, trimmed_by_refseq=False)
    # -------------------------------------------------------------- #
