...
def plot_di_compare_methods(ax, ct, di1, di2, ld_thresh, contact_dist, s_index, labels):
    ...
    mask = np.triu(np.ones(di1.shape[0], dtype=bool), k=1)
    # argsort sorts from low to high. [::-1] reverses 
    order = di1[mask].argsort()[::-1]
    ...
    di1_flat = di1[mask][order]
    di2_flat = di2[mask][order]
...
def plot_contact_map_ecc(ax2, method, pairs_flat, ct_flat, dist_flat, ER_di_flat, ct_ER, s_index, ld_val=5, title=False):
...
def plot_true_positive_rates_ecc(ct, tps, methods, contact_dist, linear_dist, ax):
    ...
    # gets pdb tp rate
    ct1 = ct.copy()
    ct_pos = ct1 < contact_dist
    ct1[ct_pos] = 1                                                                                          
    ct1[~ct_pos] = 0    
    pdb_mask = np.triu(np.ones(ct.shape[0], dtype=bool), k=1)
    pdb_order = ct[pdb_mask].argsort()[::]
    pdb_ct_flat = ct1[pdb_mask][pdb_order]
    max_rank = len(pdb_ct_flat)
    pdb_ranks = [i + 1 for i in range(max_rank)]
    pdb_tpr = [t/pdb_ranks[j] for j,t in enumerate(np.cumsum(pdb_ct_flat, dtype=float))]
...
if __name__ == '__main__':
    ...
    33172          ER   5zu4  PF03318  0.996954           True   
    10754          ER   2zso  PF00042  0.996954           True   
    18719          ER   2iya  PF00201  0.996953           True   
    12387          ER   1rwy  PF13499  0.995999           True   
    20721          ER   2eb8  PF00042  0.995949           True   
    """
    #pdb_id = sys.argv[1]
    #pfam_id = sys.argv[2]
    pdb_id = '1zdr'
    ...
    if single_pdf:
        pdf_output = matplotlib.backends.backend_pdf.PdfPages('single_protein_plots/%dseq_%dcol_%s_%s_plots.pdf' % (int(n_seq), int(n_col), pdb_id, pfam_id) )
    ...
    ax = plot_contact_map_ecc(ax, 'ER', ER_pairs_flat, ER_ct_flat, ER_dist_flat, ER_di_flat, contact_dist, s_index, ld_val=ld_thresh)
    ...
    if single_pdf:
        ...
        plt.savefig('%s_%s_di_contact_ER.pdf' % (pdb_id, pfam_id) )
    
    
    fig = plt.figure(figsize=(5, 5))
    ax = plt.subplot2grid((1,1),(0,0))
    ax = plot_contact_map_ecc(ax, 'MF', PMF_pairs_flat, PMF_ct_flat, PMF_dist_flat, PMF_di_flat, contact_dist, s_index, ld_val=ld_thresh)
    ax.legend()
    plt.tight_layout()
    if single_pdf:
        pdf_output.savefig( fig )
    else:
        plt.savefig('%s_%s_di_contact_MF.pdf' % (pdb_id, pfam_id) )
    
    fig = plt.figure(figsize=(5,5))
    ax = plt.subplot2grid((1,1),(0,0))
    ax = plot_contact_map_ecc(ax, 'PLM', PLM_pairs_flat, PLM_ct_flat, PLM_dist_flat, PLM_di_flat, contact_dist, s_index, ld_val=ld_thresh)
    ax.legend()
    plt.tight_layout()
    if single_pdf:
        pdf_output.savefig( fig )
    else:
        plt.savefig('%s_%s_di_contact_PLM.pdf' % (pdb_id, pfam_id) )
    ...
    methods = ['ER', 'MF', 'PLM']
    ct_flats = [ER_ct_flat, PMF_ct_flat, PLM_ct_flat ]
    tps = []
    ...
    fig = plt.figure(figsize=(5,5)) 
    # fig = plt.figure(figsize=(5,5.75))  # no title
    ax = plt.subplot2grid((1,1),(0,0))
    ax = plot_true_positive_rates_ecc(ct.copy(), tps, methods, contact_dist, ld_thresh, ax)
    ax.legend()
    plt.tight_layout()
    if single_pdf:
        pdf_output.savefig( fig )
    else:
        plt.savefig('%s_%s_di_tp_ecc.pdf' % (pdb_id, pfam_id) )
    ...
    # fig = plt.figure(figsize=(5,5.75))  # no title
    ax = plt.subplot2grid((1,1),(0,0))
    ax = plot_true_positive_rates_ecc(ct.copy(), [tps[0]], methods, contact_dist, ld_thresh, ax)
    ax.legend()
    plt.tight_layout()
    if single_pdf:
        pdf_output.savefig( fig )
    else:
        plt.savefig('%s_%s_ER_di_tp_ecc.pdf' % (pdb_id, pfam_id) )
    ...
    fig = plt.figure(figsize=(5,5))
    ax = plt.subplot2grid((1,1),(0,0))
    ax = plot_di_vs_ct(ax, ER_ct_flat, ER_dist_flat, flat_dis, ER_ld_flat, labels)
    plt.tight_layout()
    # ax.legend()
    if single_pdf:
        pdf_output.savefig( fig )
    ...
    fig = plt.figure(figsize=(5,5))
    ax = plt.subplot2grid((1,1),(0,0))
    ax = plot_di_compare_methods(ax, ct, dis[0], dis[1], ld_thresh, contact_dist, s_index, labels)
    ax.legend()
    ...
    fig = plt.figure(figsize=(5,5))
    ax2 = plt.subplot2grid((1,1),(0,0))
    ax2 = plot_di_compare_methods(ax2, ct, dis[0], dis[1], ld_thresh, contact_dist, s_index, labels)
    ax2.legend()
    plt.tight_layout()
    if single_pdf:
        pdf_output.savefig( fig )
    ...
    er_fpr, er_tpr, thresholds, roc_auc, ct_pos_flat = roc_curve_new(ct, ER_di, contact_dist, s_index, ld_thresh=ld_thresh, get_uniform=False)
    pmf_fpr, pmf_tpr, thresholds, roc_auc, ct_pos_flat = roc_curve_new(ct, PMF_di, contact_dist, s_index, ld_thresh=ld_thresh, get_uniform=False)
    plm_fpr, plm_tpr, thresholds, roc_auc, ct_pos_flat = roc_curve_new(ct, PLM_di, contact_dist, s_index, ld_thresh=ld_thresh, get_uniform=False)
    
    
    fig = plt.figure(figsize=(5,5))
    ax = plt.subplot2grid((1,1),(0,0))
    ax.plot(er_fpr, er_tpr, label='ER', color=colors_hex[colors_key[0]])
    ax.plot(pmf_fpr, pmf_tpr, label='MF', color=colors_hex[colors_key[1]])
    ax.plot(plm_fpr, plm_tpr, label='PLM', color=colors_hex[colors_key[2]])
    ax.set_ylabel('True Positive Rate', fontsize=14)
    ax.set_xlabel('False Positive Rate', fontsize=14)
    ax.legend()
    plt.tight_layout()
    if single_pdf:
        ...
        plt.savefig('%s_%s_roc_ecc.pdf' % ( pdb_id, pfam_id) )
