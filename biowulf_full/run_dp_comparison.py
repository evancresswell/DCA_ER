...
def gen_ER_ROC(ct_mat, ER_di, pdb_id, pfam_id, file_end='.npy', gen_uniform_roc=False, method='ER'):
    # ER ROC
    auc_ER = np.zeros(n)
    for i in range(n):
        try:
    ...
    print('%s tp raw (len %d): ' % (method, len(tpr0_ER)), tpr0_ER[:10])
...
print('Final MF DI shape (cols removed): ', er_di.shape)
if remove_diagonals: 
    MF_di = no_diag(er_di, 4, s_index)
else:
    MF_di = er_di

from ecc_tools import scores_matrix2dict
print(s_index)
#MF_di_dict = scores_matrix2dict(MF_di, s_index, removed_cols_range, removing_cols=removing_cols)
#print(MF_di_dict[:20])
...
# ------------------------------------------------------------------------------------------------------------------- #
# -------------------------------- Generate TP/FP Date -------------------------------------------------------------- #
MF_fp, MF_tp, MF_ct_thres = gen_ER_ROC(ct_mat, MF_di, pdb_id, pfam_id, file_end='.npy', gen_uniform_roc=False)

PMF_di, PMF_fp, PMF_tp, PMF_ct_thres, PMF_auc, PMF_ct_pos_flat = gen_PYDCA_ROC(ct_mat, MF_di, s_index, PMF_di_data, pdb_id, pfam_id, 'PMF', file_end='.npy', gen_uniform_roc=False)
...
print('di shapes:\nMF: %d\nPMF: %d' % (MF_di.shape[1],PMF_di.shape[1]))
from math import comb

print('pair combos (sample size): %d' % comb(MF_di.shape[1],2))
print('NOTE: Scikit-learn\'s ROC curve returncs tpr, fpr arrays <= len of observations..\ntp lengths:\nMF: %d\nPMF: %d' % (len(MF_tp), len(PMF_tp)))


from generate_average_roc import get_average_roc
results,cdfs = ks_compare_roc_asymptotic(tprs, fprs, methods, comb(MF_di.shape[1],2))
print(results)
...
# Set MF as the reference order for the contact-binary array
di_not = MF_di
...
MF_di_compare = di_not[mask][order][ld_flat]
...
method_flat_di = [MF_di_compare, PMF_di_compare]
...
result_file = "%s/%s_%s_dp_comparison.pkl" % (out_metric_dir, pdb_id, pfam_id)
...
pfam_dimensions = [n_var, n_seq, meff, MF_ct_thres,  PMF_ct_thres]
pfam_dimensions_file = "%s%s_%s_dp_pfam_dimensions%s" % (processed_data_dir, pdb_id, pfam_id, file_end)
